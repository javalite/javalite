/*
Copyright 2009-2010 Igor Polevoy 

Licensed under the Apache License, Version 2.0 (the "License"); 
you may not use this file except in compliance with the License. 
You may obtain a copy of the License at 

http://www.apache.org/licenses/LICENSE-2.0 

Unless required by applicable law or agreed to in writing, software 
distributed under the License is distributed on an "AS IS" BASIS, 
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. 
See the License for the specific language governing permissions and 
limitations under the License. 
*/


package activejdbc.oracle_test;

import activejdbc.LazyList;
import activejdbc.Model;
import activejdbc.Paginator;
import activejdbc.oracle_test.test_models.Page;
import javalite.test.jspec.ExceptionExpectation;
import org.junit.Test;


import java.util.List;
import static javalite.test.jspec.JSpec.*;

/**
 * @author Igor Polevoy
 */
public class OraclePaginatorTest extends OracleTest {

    @Override
    public void before() throws Exception {
        super.before();
        for (int i = 1; i <= 1000; i++) {
            Page.<Model>create("description", "description: " + i).saveIt();
        }

    }

    @Test
    public void testPageCount(){
        Paginator p = new Paginator(Page.class, 10, "description like '%2%'");
        a(p.pageCount()).shouldBeEqual(28);
    }

    @Test
    public void testGetPage(){
        Paginator p = new Paginator(Page.class, 10, "description like ?", "%2%").orderBy("id");
        List<Page> items = p.getPage(28);
        a(items.size()).shouldBeEqual(1);
        a(items.get(0).get("id")).shouldBeEqual(992);

       Paginator p1 = new Paginator(Page.class, 10, "*").orderBy("id");
        items = p1.getPage(2);
        a(items.size()).shouldBeEqual(10);
        a(items.get(0).get("id")).shouldBeEqual(11);//page start
        a(items.get(9).get("id")).shouldBeEqual(20);//page end
    }

    @Test
    public void testThrowExceptionIfWrongArgument(){

        final Paginator p1 = new Paginator(Page.class, 10, "*").orderBy("id");

        expect(new ExceptionExpectation(IllegalArgumentException.class) {
            public void exec() {
                p1.getPage(-2);
            }
        });
    }

    @Test
    public void testPreviousAndNext(){
        //for this query we have only 28 pages
        Paginator p = new Paginator(Page.class, 10, "description like '%2%'");

        p.getPage(27);
        a(p.hasNext()).shouldBeTrue();

        p.getPage(28);
        a(p.hasNext()).shouldBeFalse();
        a(p.hasPrevious()).shouldBeTrue();

        p.getPage(2);
        a(p.hasPrevious()).shouldBeTrue();

        p.getPage(1);
        a(p.hasPrevious()).shouldBeFalse();
        a(p.hasNext()).shouldBeTrue();

        p = new Paginator(Page.class, 10, "*").orderBy("id");
        a(p.<Model>getPage(1).size()).shouldBeEqual(10);

        LazyList<Model> page2 = p.getPage(2);
        a(page2.get(0).getId()).shouldBeEqual(11);

        System.out.println("Page 1");
        p.getPage(1).dump(System.out);
        System.out.println("Page 2");
        p.getPage(2).dump(System.out);
        System.out.println("Page 3");
        p.getPage(3).dump(System.out);
    }
}