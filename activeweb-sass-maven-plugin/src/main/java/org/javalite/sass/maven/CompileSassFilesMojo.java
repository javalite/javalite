...
public class CompileSassFilesMojo extends AbstractMojo {
    ...
    public final void execute() throws MojoExecutionException {
        ...
        if ((sassMain == null || targetDirectory == null || targetFileName == null) && sassConfigs == null
                || sassMain != null && targetDirectory != null && targetFileName != null && sassConfigs != null) {
            throw new MojoExecutionException("You must provide configuration for one less file or for a list, but not both");
        }


        if(sassConfigs == null){
            processFile(sassMain, targetDirectory, targetFileName, null);
        }else{
            for (SassConfig config : sassConfigs) {
                processFile(config.getSassMain(), config.getTargetDirectory(), config.getTargetFileName(), config.getSassArguments());
            }
        }
    }
