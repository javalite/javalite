...
public class DB implements Closeable{
    ...
    public DB open(){
        ...
        if(connectionConfigs.isEmpty()){
            throw new DBException("Could not find configuration in a property file for environment: " + AppConfig.activeEnv() +
                    ". Are you sure you called org.javalite.activejdbc.connection_config.DBConfiguration.loadConfiguration(\"/database.properties\") " +
                    "or similar? You can also call org.javalite.activejdbc.connection_config.DBConfiguration.addConnectionConfig(...) directly");
        }
     ...
     *
     * <b>The JDBC fetch size defaults to 10 records. This means that the driver will fetch and accumulate 10 records in memory. Once you read them, the driver will fetch the next 10, and so on.
     * If you want a finer control over a fetch size, use the {@link #find(RowProcessor.ResultSetType, RowProcessor.ResultSetConcur, int, String, Object...)} method
     * </b>
     *
    ...
    public RowProcessor find(String query, Object... params) {
        return find(RowProcessor.ResultSetType.FORWARD_ONLY, RowProcessor.ResultSetConcur.READ_ONLY, 10,  query, params);
    }
     ...
     * @param fetchSize hint to the JDBC driver for the size of result set. Gives the JDBC driver a hint as to the number of rows that should be fetched
     *                  from the database when more rows are needed for ResultSet objects generated by this Statement.
     *                  If the value specified is zero, then the hint is ignored and an entire set maybe loaded into your heap space - be careful out there!
     * @param params list of parameters if query is parametrized.
     * @return instance of <code>RowProcessor</code> which has with() method for convenience.
     */
