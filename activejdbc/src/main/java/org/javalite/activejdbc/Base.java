...
public class Base {
     ...
     * @param fetchSize hint to the JDBC driver for the size of result set. Gives the JDBC driver a hint as to the number
     *                  of rows that should be fetched from the database when more rows are needed for ResultSet objects
     *                  generated by this Statement. If the value specified is zero, then the hint is ignored
     *                  and an entire set maybe loaded into your heap space - be careful out there!
     * @param params list of parameters if query is parametrized.
     * @return instance of <code>RowProcessor</code> which has with() method for convenience.
     */
    ...
    public static <T> T withDb(String jndiName, Properties jndiProperties, Supplier<T> supplier) {
        return new DB(DB.DEFAULT_NAME).withDb(jndiName, jndiProperties, supplier);
    }


    /**
     * Same as {@link DB#withDb(DataSource, Supplier)}, but with db name {@link DB#DEFAULT_NAME}.
     */
    public static <T> T withDb(DataSource dataSource, Supplier<T> supplier) {
        return new DB(DB.DEFAULT_NAME).withDb(dataSource, supplier);
    }
