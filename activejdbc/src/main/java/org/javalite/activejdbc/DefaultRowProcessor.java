...
public class DefaultRowProcessor implements RowProcessor{
    ...
    protected DefaultRowProcessor(String dbName, ResultSet rs, QueryHolder queryHolder){
    	this.dbName = dbName;
        this.rs = rs;
    ...
    protected void processRS(RowListener listener) throws SQLException {
    	List<HashMap<String, Object>> cachedResult = new ArrayList<HashMap<String, Object>>();
        ...
        while (rs.next()) {
          ...
          //TODO Add hook for onLoad?? 
            ...
            cachedResult.add(row);
        }
        putInSqlCache(cachedResult);
        rs.close();
    }

	private void putInSqlCache(List<HashMap<String, Object>> cachedResult) {
		if(ConnectionsAccess.isSqlQueryCacheEnabled(dbName)){
        	ConnectionsAccess.putCachedResult(dbName, queryHolder, cachedResult);
        }
	}
}
