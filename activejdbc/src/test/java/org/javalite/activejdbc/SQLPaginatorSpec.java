 ...
 * @author Igor Polevoy
 */
public class SQLPaginatorSpec extends ActiveJDBCTest {
    ...
    public void testCurrentPage(){
        ...
        SQLPaginator sp = SQLPaginator.instance()
                .query("select * from items where item_number > ? order by item_number")
                .params(20)
                .countQuery("select * from items where item_number > ? order by item_number")
                .countQueryParams(20)
                .pageSize(10).get();

        a(sp.getCurrentPage()).shouldBeEqual(0);
        sp.getPage(1);
        a(sp.getCurrentPage()).shouldBeEqual(1);
        List<Map> items = sp.getPage(1);
