...
public class PostgreSQLDialectTest extends ActiveJDBCTest {
    ...
    public void testFormSelectWithLimitOffset() {
        a(dialect.formSelect("people", null, null, new ArrayList<>(), 1, -1)).shouldBeEqual("SELECT * FROM people LIMIT 1");
        a(dialect.formSelect("people", null, null, new ArrayList<>(), -1, 1)).shouldBeEqual("SELECT * FROM people OFFSET 1");
        a(dialect.formSelect("people", null, null, new ArrayList<>(), 1, 1)).shouldBeEqual("SELECT * FROM people LIMIT 1 OFFSET 1");
        a(dialect.formSelect("people", null, "last_name = ?", new ArrayList<>(), 1, 10)).shouldBeEqual("SELECT * FROM people WHERE last_name = ? LIMIT 1 OFFSET 10");
